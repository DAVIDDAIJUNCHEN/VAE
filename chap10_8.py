...
weights = {
        ...
        'mean_w1': tf.Variable(tf.truncated_normal([n_hidden_1, n_hidden_2], stddev=0.01)),
        'mean_b1': tf.Variable(tf.zeros([n_hidden_2])),
        'log_sigma_w1': tf.Variable(tf.truncated_normal([n_hidden_1, n_hidden_2], stddev=0.01)),
        'log_sigma_b1': tf.Variable(tf.zeros(n_hidden_2)),
        'w2': tf.Variable(tf.truncated_normal([n_hidden_2, n_hidden_1], stddev=0.01)),
        'b2': tf.Variable(tf.zeros([n_hidden_1])),
        'w3': tf.Variable(tf.truncated_normal([n_hidden_1, n_input], stddev=0.01)),
...
h2out = tf.nn.relu(tf.matmul(z_input, weights['w2']) + weights['b2'])
...
with tf.Session() as sess:
    ...
    reconstruction = sess.run(reconstruction,
            feed_dict = {x:mnist.test.images[10:(10+show_num)]})
    ...
    for i, yi in enumerate(grid_x):
        for j, xj in enumerate(grid_y):
            ...
            x_decoded = sess.run(reconstructionout, feed_dict={z_input:z_sample})
